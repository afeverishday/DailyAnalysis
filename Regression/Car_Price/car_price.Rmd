...
마지막으로 다변량 데이터에서 이상치를 판단하는 방법에 대하여 살펴보자. 다변량데이터(물론 이변량도 포함된다.)에서 이상치를 판단하는 방법은 마할로노비스 거리를 측정하는 방법, Local Outlier Factor를 이용하는 방법 등이 존재한다 여기서는 위 두가지의 사용했다. 어디까지나 예시로 사용해본 것이고, 실제 이를 사용함에 있어 도메인 지식이 활용되어야 한다. 
