...
# 1. 데이터셋 생성하기
rawdata = pd.DataFrame( pd.read_csv('Hotel/수원라마다호텔(사우나).csv'))\
.rename(columns={'검침일(시간)' : 'time', '용도'  : 'gas_type', '비보정사용량' : 'raw_usage', '보정사용량': 'calib_usage', 
                      ...
                      .assign(time=lambda x: pd.to_datetime(x['time']),
                              calib_usage=lambda x: x['calib_usage'].str.replace(',','').astype('float'))[['time','calib_usage']].dropna()\
                      .query("time>= '2018-11-01 00:00' and time< '2019-02-01 00:00'").drop('time',axis=1)

# hands6 1-2, 2-1, 2-3, 2-4
rawdata = pd.DataFrame( pd.read_csv('hands6/heat/핸즈식스1공장_2번.csv'))\
.rename(columns={'검침일(시간)' : 'time', '용도'  : 'gas_type', '비보정사용량' : 'raw_usage', '보정사용량': 'calib_usage', 
                                        '보정계수':'calib_const', '순간유량': 'flux'}).drop(0,axis=0)\
                      .assign(time=lambda x: pd.to_datetime(x['time']),
                              calib_usage=lambda x: x['calib_usage'].str.replace(',','').astype('float'))[['time','calib_usage']].dropna()\
...
.rename(columns={'검침일(시간)' : 'time', '용도'  : 'gas_type', '비보정사용량' : 'raw_usage', '보정사용량': 'calib_usage',
                      ...
                      .assign(time=lambda x: pd.to_datetime(x['time']),
                              ...
                              calib_usage=lambda x: x['calib_usage'].astype('float')).dropna()\
...
rawdata =rawdata.query("month in ['11','12','1','2']")[['time','calib_usage']].dropna().set_index('time')
...
# 데이터 분리( per a day)
train = signal_data[0:len(rawdata.query("time <= '2020-01-09 00:00'"))] # rawdata.index[0:round(len(signal_data)*0.6)]
val = signal_data[len(rawdata.query("time > '2020-01-09 00:00'")):len(rawdata.query("time <= '2020-01-01 00:00'"))] # rawdata.index[round(len(signal_data)*0.6):round(len(signal_data)*0.95)]
test = signal_data[len(rawdata.query("time > '2020-01-09 00:00'")):] # rawdata.index[round(len(signal_data)*0.955):]
test = signal_data[-(look_back+look_ahead):]
#apt_rawdata[round(-274:]
# 데이터셋 생성
...
for i in range(look_ahead):
    prediction = multiperceptron_model.predict(xhat, batch_size=48)
